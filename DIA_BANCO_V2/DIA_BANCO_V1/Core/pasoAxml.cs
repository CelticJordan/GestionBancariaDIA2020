...

{
     ...
     */
        ...
        /// </summary>
        public XElement ToXmlCuenta(Cuenta cuenta)
        {
            XElement toret = new XElement(EtiquetaCuenta);
            toret.Add(
                //guardado de atributos
                new XAttribute(EtiquetaCCC, cuenta.GetCCC.ToString()),
                new XAttribute(EtiquetaTipo, cuenta.GetTipo.ToString()),
                new XAttribute(EtiquetaSaldo, cuenta.GetSaldo.ToString()),
                new XAttribute(EtiquetaFechaApertura, cuenta.GetFechaApertura.ToString()),
                new XAttribute(EtiquetaInteresMensual, cuenta.GetInteresMensual.ToString())
            );
                ...
                foreach (string cl in cuenta.GetTitulares)
                {
                    clientesx.Add(new XAttribute(EtiquetaTitular, cl.ToString()));
                }
                toret.Add(clientesx);
                ...
                foreach (Cuenta.Deposito d in cuenta.GetDepositos)
                {
                ...
                foreach (Cuenta.Retirada r in cuenta.GetRetiradas)
                {
        ...
        /// </summary>
        public XElement ToXmlCliente(Cliente cliente)
        {
            XElement toret = new XElement(EtiquetaCliente);
            toret.Add(
                new XAttribute(EtiquetaDni, cliente.GetDni.ToString()),
                new XAttribute(EtiquetaNombre, cliente.GetNombre.ToString()),
                new XAttribute(EtiquetaTelefono, cliente.GetTelefono.ToString()),
                new XAttribute(EtiquetaEmail, cliente.GetEmail.ToString()),
                new XAttribute(EtiquetaDireccion, cliente.GetDirPostal.ToString())
            );
        ...
        /// </summary>
        public XElement ToXmlRetirada(Cuenta.Retirada retirada)
        {
            XElement toret = new XElement(EtiquetaRetirada);
            toret.Add(
                
                new XAttribute(EtiquetaConcepto, retirada.GetConcepto.ToString()),
                new XAttribute(EtiquetaCantidad, retirada.GetCantidad.ToString()),
                new XAttribute(EtiquetaDateTime, retirada.GetDateTime.ToString())
            );
            return toret;
        }
        ...
        /// </summary>
        public XElement ToXmlDeposito(Cuenta.Deposito deposito)
        {
            XElement toret = new XElement(EtiquetaDeposito);
            toret.Add(
                new XAttribute(EtiquetaConcepto, deposito.GetConcepto.ToString()),
                new XAttribute(EtiquetaCantidad, deposito.GetCantidad.ToString()),
                new XAttribute(EtiquetaDateTime, deposito.GetDateTime.ToString())
            );
            return toret;
        }
        ...
        /// </summary>
        public XElement ToXmlPrestamo(Prestamo prestamo)
        {
            XElement toret = new XElement(EtiquetaPrestamo);
            toret.Add(
                new XAttribute(EtiquetaId, prestamo.GetIdP.ToString()),
                new XAttribute(EtiquetaCCCorigen, prestamo.GetCccOri.ToString()),
                new XAttribute(EtiquetaCCCdestino, prestamo.GetCccDes.ToString()),
                new XAttribute(EtiquetaImporte, prestamo.GetImporte.ToString()),
                new XAttribute(EtiquetaFecha, prestamo.GetFecha.ToString())
        ...
        /// </summary>
        public XElement ToXmlTransferencia(Transferencia transferencia)
        {
            XElement toret = new XElement(EtiquetaTransferencia);
            toret.Add(
                new XAttribute(EtiquetaId, transferencia.GetId.ToString()),
                new XAttribute(EtiquetaTipoTransferencia, transferencia.GetTipo.ToString()),
                new XAttribute(EtiquetaCCCorigen, transferencia.GetCCCOrigen.ToString()),
                new XAttribute(EtiquetaCCCdestino, transferencia.GetCCCDestino.ToString()),
                new XAttribute(EtiquetaImporte, transferencia.GetImporte.ToString()),
                new XAttribute(EtiquetaFecha, transferencia.GetFecha.ToString())
            );
        ...
        /// </summary>
        public void GuardaClientesXml(List<Clientes> contenedorClientes, String archivo)
        {
        ...
        /// </summary>
        public void GuardaXml(List<Transferencia> contenedorTransferencias, String archivo)
        {
        ...
        /// </summary>
        public void CargarCuentasXml(List<Cuenta> contenedorCuentas, String archivo)
        {
            XDocument documento;
            try
            {
            ...
            catch (System.IO.FileNotFoundException) //si no encuentra el archivo creamos uno vacio
            {
            ...
            if (documento.Root != null && documento.Root.Name == EtiquetaBancoCuentas) //en el caso de que encuentre un documento escrito
            {
                ...
                foreach (var cu in cuentas)
                {
                    ...
                    //lista de depositos
                    List<Cuenta.Deposito> depositos = null;
                    ...
                    //lista de retiradas
                    List<Cuenta.Retirada> retiradas = null;
                    ...
                    if (tipo == "Ahorro"){
                         cuenta = new CuentaAhorro(ccc, tipo, saldo, fechaApertura, interes, titulares, depositos, retiradas, transferencias, prestamos);

                    }else if(tipo == "Vivienda"){
                         cuenta = new CuentaVivienda(ccc, tipo, saldo, fechaApertura, interes, titulares, depositos, retiradas, transferencias, prestamos);

                    }else if (tipo == "Corriente"){
                         cuenta = new CuentaCorriente(ccc, tipo, saldo, fechaApertura, interes, titulares, depositos, retiradas, transferencias, prestamos);
        ...
        /// </summary>
        public void CargarPrestamosXml(List<Prestamo> contenedorPrestamos, String archivo)
        {
            XDocument documento;
            try
            {
            ...
            {
                documento = new XDocument();
                Console.WriteLine("No se han encontrado datos anteriores, se ha creado un archivo nuevo \n");
            }
        ...
        /// </summary>
        public void CargarClientesXml(List<Cliente> contenedorClientes, String archivo)
        {
            XDocument documento;
            try
            {
            ...
            if (documento.Root != null && documento.Root.Name == EtiquetaBancoClientes) //en el caso de que encuentre un documento escrito
            {
                    ...
                    var clientes = documento.Root.Elements(EtiquetaBancoClientes);
        ...
        /// </summary>
        public void CargarTransferenciasXml(List<Transferencia> contenedorTransferencias, String archivo)
        {
            XDocument documento;
            try
            {
            ...
            if (documento.Root != null && documento.Root.Name == EtiquetaBancoClientes) //en el caso de que encuentre un documento escrito
            {
                    ...
                    var transferencias = documento.Root.Elements(EtiquetaBancoTransferencias);
                    foreach (var tr in transferencias)
                    {
                        ...
                        Transferencia transferencia = new Transferencia(id, tipoTransferencia, cccOrigen, cccDestino, importe, fecha);
